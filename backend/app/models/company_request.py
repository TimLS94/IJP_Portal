...
class CompanyRequest(Base):
    ...
    # Art des Auftrags
    request_type = Column(Enum(CompanyRequestType, values_callable=lambda x: [e.value for e in x]), nullable=False)
    
    # Status
    status = Column(Enum(CompanyRequestStatus, values_callable=lambda x: [e.value for e in x]), default=CompanyRequestStatus.PENDING)
