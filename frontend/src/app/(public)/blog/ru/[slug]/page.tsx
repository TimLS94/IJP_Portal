...
async function getRelatedPosts(category: string, currentSlug: string): Promise<BlogPost[]> {
  try {
    const res = await fetch(`${API_URL}/blog/posts?category=${category}&language=ru&limit=4`, {
      next: { revalidate: 300 },
    });
    if (!res.ok) return [];
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const pageUrl = `https://www.jobonportal.de/blog/ru/${post.slug}`;
  return {
...
export default async function BlogDetailRuPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const post = await getBlogPost(slug);
  ...
  return (
    <>
      ...
      />
      <BlogDetailClient post={post} relatedPosts={relatedPosts} language="ru" />
      <BlogJobsCta lang="ru" />
    </>
  );
}
