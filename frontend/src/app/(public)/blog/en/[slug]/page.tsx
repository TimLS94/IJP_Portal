...
async function getRelatedPosts(category: string, currentSlug: string): Promise<BlogPost[]> {
  try {
    const res = await fetch(`${API_URL}/blog/posts?category=${category}&language=en&limit=4`, {
      next: { revalidate: 300 },
    });
    if (!res.ok) return [];
...
export default async function BlogDetailEnPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const post = await getBlogPost(slug);
  ...
  return (
    <>
      ...
      />
      <BlogDetailClient post={post} relatedPosts={relatedPosts} language="en" />
    </>
  );
}
