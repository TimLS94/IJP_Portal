...
interface ApplicationDetails {
  ...
  applicant: {
    ...
    semester_break_start?: string;
    semester_break_end?: string;
    address: {
      city?: string;
      country?: string;
    };
...
export default function AdminApplicationsPage() {
  ...
  useEffect(() => {
    loadApplications();
  }, [statusFilter, positionTypeFilter, inviteSourceFilter, page, limit]);

  const loadInviteSources = async () => {
    try {
      const response = await adminAPI.getInviteSources();
      setInviteSources(response.data.sources || []);
    } catch {
      console.error("Fehler beim Laden der Einladungsquellen");
    }
  };
  ...
  const handleDownloadDocuments = async (applicantId: number, applicantName: string) => {
    try {
  ...
  const handleDownloadSingleDocument = async (documentId: number, fileName: string) => {
    try {
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      <div className="card mb-6">
        ...
        {(statusFilter || positionTypeFilter || inviteSourceFilter) && (
          <div className="flex items-center gap-2 mt-4 pt-4 border-t">
            ...
            <button 
              onClick={() => { setStatusFilter(""); setPositionTypeFilter(""); setInviteSourceFilter(""); }}
                          ...
                          applicationDetails.applicant.semester_break_start || applicationDetails.applicant.semester_break_end ? (
                            <div className="mt-2 p-2 bg-green-50 rounded border border-green-200">
                              <p className="text-green-700 font-medium">
                                ...
                                {applicationDetails.applicant.semester_break_start ? formatDate(applicationDetails.applicant.semester_break_start) : "?"} 
                                {" - "}
                                {applicationDetails.applicant.semester_break_end ? formatDate(applicationDetails.applicant.semester_break_end) : "?"}
                              </p>
                            </div>
                          ) : (
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="bg-gray-50 rounded-xl p-4">
                      ...
                      <div className="space-y-2 text-sm mb-4">
                        ...
                        <span className={`inline-block px-3 py-1 rounded-full text-sm font-medium ${statusColors[statusOptions.find(s => s.value === applicationDetails.application.status)?.color || "gray"] || statusColors.gray}`}>
