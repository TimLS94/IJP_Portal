...
export default function CreateJobPage() {
  ...
  const onSubmit = async (data: Record<string, unknown>, isDraft = false) => {
    ...
    if (!isDraft && !translations.de.description?.trim()) { toast.error("Bitte Beschreibung eingeben"); return; }
    setSaving(true);
    try {
      const d: Record<string, unknown> = { ...data, title: translations.de.title, description: translations.de.description, tasks: translations.de.tasks, requirements: translations.de.requirements, benefits: translations.de.benefits, position_type: selectedPositionTypes[0] || null, position_types: selectedPositionTypes, available_languages: enabledLanguages, is_draft: isDraft, is_active: !isDraft, other_languages_required: otherLanguages.filter(l => l.language) };
